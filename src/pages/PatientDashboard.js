...
import { Calendar, Clock, Award, TrendingUp, LogOut, Activity, Stethoscope, HeartPulse, CircleDot, Search, User, Plus, ChevronRight, Flame, ArrowUpRight, ShieldCheck, } from "lucide-react";
...
const initials = (name) => (name || "?")
    ...
    .join("")
    .toUpperCase();
const statusTone = (status) => status === "upcoming"
...
const PatientDashboard = () => {
    ...
    const todays = useMemo(() => appointments.filter((a) => { try {
        return isToday(parseISO(a.appointment_date));
    }
    catch {
        return false;
    } }), [appointments]);
    const nextAppt = useMemo(() => appointments.find((a) => a.status === "upcoming") || null, [appointments]);
    const stats = [
    ...
    return (<div className="relative flex min-h-screen w-full flex-1 flex-col overflow-x-hidden bg-background">
      <Seo title="Your Dashboard — HealthBook" description="Track your health score, upcoming appointments, and achievements in your HealthBook dashboard." path="/patient-dashboard"/>

      {/* Ambient background */}
      <div aria-hidden className="pointer-events-none absolute inset-x-0 top-0 h-[420px] opacity-[0.55]" style={{
            background: "radial-gradient(60% 100% at 15% 0%, hsl(var(--primary) / 0.16), transparent 70%), radial-gradient(50% 100% at 85% 0%, hsl(var(--stat-cyan) / 0.14), transparent 70%)",
        }}/>
      ...
      <nav className="sticky top-0 z-50 border-b border-border/60 bg-background/70 backdrop-blur-xl">
        <div className="container mx-auto flex items-center justify-between gap-3 px-4 py-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {isAdmin && (<Button asChild variant="secondary" size="sm" className="h-9 rounded-full px-4 text-xs font-semibold">
      ...
      <main className="container relative mx-auto space-y-5 px-4 py-6">
        ...
        <section className="grid grid-cols-2 gap-3 xl:grid-cols-4">
          ...
          {stats.map((s) => (<article key={s.label} className="group relative flex min-h-[142px] flex-col justify-between overflow-hidden rounded-[22px] border border-border/70 bg-card p-4 shadow-[0_1px_2px_hsl(var(--foreground)/0.04)] transition-all duration-300 hover:-translate-y-1 hover:border-primary/25 hover:shadow-large">
              <span aria-hidden className="absolute -right-8 -top-10 h-24 w-24 rounded-full opacity-0 blur-2xl transition-opacity duration-300 group-hover:opacity-100" style={{ background: `hsl(var(--${s.tint}) / 0.35)` }}/>
              <div className="relative flex items-start justify-between gap-2">
                <span className="flex h-9 w-9 items-center justify-center rounded-xl ring-1 ring-inset" style={{
                ...
                {s.badge && (<span className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-muted-foreground">
                    {s.badge}
                  </span>)}
              </div>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
          {/* Appointments list */}
          <WidgetCard className="self-start rounded-[22px] lg:col-span-8" title="Your Appointments" description={`${appointments.length} total · ${upcoming} upcoming`} icon={Calendar} tint="stat-blue" action={{ label: "Book new", to: "/doctors" }}>
            {appointments.length === 0 ? (<div className="py-10 text-center">
              ...
              </div>) : (<div className="grid grid-cols-1 gap-2.5 xl:grid-cols-2">
                {appointments.map((a, i) => {
                const tint = ["stat-blue", "stat-cyan", "stat-violet", "stat-green", "stat-amber", "stat-red"][i % 6];
                return (<div key={a.id} className="group/appt relative flex items-center gap-3 overflow-hidden rounded-2xl border border-border/70 bg-card px-3.5 py-3 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-medium">
                      <span aria-hidden className="absolute inset-y-0 left-0 w-[3px] opacity-70" style={{ background: `hsl(var(--${tint}))` }}/>
                      <div className="ml-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-[11px] font-bold overflow-hidden" style={{ background: `hsl(var(--${tint}) / 0.12)`, color: `hsl(var(--${tint}))` }}>
                        {a.doctors?.avatar_url ? (<img src={a.doctors.avatar_url} alt={a.doctors.full_name} className="w-full h-full object-cover" onError={(e) => {
                            e.target.style.display = 'none';
                            e.target.parentElement.innerText = initials(a.doctors?.full_name);
          ...
          <div className="space-y-4 lg:col-span-4">
            <WidgetCard className="rounded-[22px]" title="Next Visit" description="Your closest appointment" icon={Clock} tint="stat-cyan">
              {!nextAppt ? (<EmptyState label="No upcoming visit scheduled"/>) : (<div className="relative overflow-hidden rounded-2xl bg-gradient-primary p-5 text-primary-foreground shadow-medium">
                  ...
                  <div className="relative mt-3 flex items-center gap-3">
                    <div className="h-8 w-8 shrink-0 rounded-full border border-white/20 overflow-hidden bg-white/10 flex items-center justify-center text-[10px] font-bold">
                      {nextAppt.doctors?.avatar_url ? (<img src={nextAppt.doctors.avatar_url} alt={nextAppt.doctors.full_name} className="w-full h-full object-cover" onError={(e) => {
                    e.target.style.display = 'none';
                    e.target.parentElement.innerText = initials(nextAppt.doctors?.full_name);
            ...
            <WidgetCard className="rounded-[22px]" title="Today's Schedule" description="Visits happening today" icon={Activity} tint="stat-violet">
              {todays.length === 0 ? (<EmptyState label="Nothing scheduled today"/>) : (<ol className="relative space-y-2.5 pl-[58px]">
                  <span className="absolute left-[50px] top-1 bottom-1 w-px bg-border"/>
                  {todays.map((a) => (<li key={a.id} className="relative">
            ...
            <WidgetCard className="rounded-[22px]" title="Quick Actions" description="Get things done faster" icon={TrendingUp} tint="stat-amber">
              <div className="grid grid-cols-2 gap-2.5">
                {quickActions.map((q) => (<Link key={q.label} to={q.to} className="flex flex-col items-start gap-2.5 rounded-2xl border border-border/70 bg-muted/25 p-3.5 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/25 hover:bg-card hover:shadow-medium">
                    <span className="flex h-9 w-9 items-center justify-center rounded-xl" style={{ background: `hsl(var(--${q.tint}) / 0.12)`, color: `hsl(var(--${q.tint}))` }}>
        ...
        ].map((b) => (<li key={b.label} className="flex items-center gap-2.5 rounded-xl px-2 py-2 transition-colors hover:bg-muted/60">
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl" style={{ background: `hsl(var(--${b.tint}) / 0.12)`, color: `hsl(var(--${b.tint}))` }}>
