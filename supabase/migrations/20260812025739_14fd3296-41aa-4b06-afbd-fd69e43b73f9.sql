CREATE OR REPLACE FUNCTION public.create_guest_booking(
  ...
  _doctor_id uuid DEFAULT NULL::uuid,
  _date date DEFAULT NULL::date,
  _time text DEFAULT NULL::text,
  _reason text DEFAULT NULL::text,
  _service_id uuid DEFAULT NULL::uuid,
  _reason_id uuid DEFAULT NULL::uuid,
  _custom_reason text DEFAULT NULL::text
)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  VALUES (_user_id, _first_name || ' ' || _last_name, _phone, 'patient')
  ON CONFLICT (id) DO UPDATE
  SET full_name = EXCLUDED.full_name,
      phone = EXCLUDED.phone;

  -- 2. Ensure patient_intake exists
  INSERT INTO public.patient_intake (id, first_name, last_name, email, phone, date_of_birth, gender)
