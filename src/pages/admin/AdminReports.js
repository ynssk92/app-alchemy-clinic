...
import { Users, Calendar, Activity, FileText, Download, Printer, RefreshCw, Stethoscope, CalendarCheck, BarChart3 } from "lucide-react";
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, CartesianGrid, Legend } from "recharts";
import { WidgetCard, EmptyState } from "@/components/dashboard/WidgetCard";
import { KpiCard } from "@/components/dashboard/KpiCard";
import { format, subDays, startOfMonth, endOfMonth, startOfWeek, endOfWeek, subMonths, startOfYear, endOfYear, parseISO } from "date-fns";
...
export const AdminReports = () => {
    ...
    return (<div className="space-y-8 animate-in fade-in duration-500 pb-10 p-6 max-w-7xl mx-auto">
      {/* Top Bar */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard label="Total Patients" value={stats.totalPatients} subtitle={`+${stats.newPatients} cette période`} icon={Users} tint="stat-blue" up={true} delta={`${((stats.newPatients / (stats.totalPatients || 1)) * 100).toFixed(1)}%`}/>
        <KpiCard label="Rendez-vous" value={stats.totalAppts} subtitle={`${stats.upcomingAppts} à venir`} icon={Calendar} tint="stat-cyan"/>
        <KpiCard label="Consultations" value={stats.completedAppts} subtitle={`${((stats.completedAppts / (stats.totalAppts || 1)) * 100).toFixed(0)}% de complétion`} icon={Stethoscope} tint="stat-green"/>
        <KpiCard label="Ordonnances" value={stats.totalPrescriptions} subtitle="Générées cette période" icon={FileText} tint="stat-violet"/>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Appointments by Status */}
        <WidgetCard className="lg:col-span-4" title="Statut des Rendez-vous" description="Répartition par état actuel" icon={Activity} tint="stat-cyan">
          <div className="h-[250px] mt-4">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={apptsByStatus} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                  {apptsByStatus.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                </Pie>
        ...
        <WidgetCard className="lg:col-span-8" title="Activité par Type de Service" description="Volume de consultations par catégorie" icon={BarChart3} tint="stat-blue">
          <div className="h-[250px] mt-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={apptsByType}>
                ...
                <Tooltip cursor={{ fill: 'hsl(var(--muted)/0.4)' }} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}/>
      ...
      <WidgetCard title="Activité des Praticiens" description="Performance par docteur sur la période sélectionnée" icon={Users} tint="stat-violet">
        <div className="mt-4 border border-border/50 rounded-xl overflow-hidden bg-card/50">
          <Table>
            ...
            <TableBody>
              {doctorActivity.length === 0 ? (<TableRow>
                  <TableCell colSpan={5} className="h-32 text-center text-muted-foreground">
                    Aucune donnée pour cette période
                  </TableCell>
                </TableRow>) : (doctorActivity.map((doc, i) => (<TableRow key={i} className="hover:bg-muted/30 transition-colors">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <WidgetCard title="Derniers Rendez-vous" description="Aperçu des rendez-vous récents" icon={CalendarCheck} tint="stat-cyan">
          <div className="space-y-4 mt-2">
            {filteredData.appointments.slice(0, 5).map((a, i) => (<div key={i} className="flex items-center justify-between p-3 rounded-xl border border-border/50 hover:border-primary/20 transition-all hover:bg-primary/5 group">
                ...
                <div className="text-right">
                  ...
                  <Badge className={`text-[10px] h-5 ${a.status === 'completed' ? 'bg-emerald-100 text-emerald-700' :
                a.status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'} border-none`}>
                    {a.status}
                  </Badge>
                </div>
              </div>))}
        ...
        <WidgetCard title="Dernières Ordonnances" description="Activité de prescription récente" icon={FileText} tint="stat-violet">
          <div className="space-y-4 mt-2">
            {filteredData.prescriptions.slice(0, 5).map((p, i) => (<div key={i} className="flex items-center justify-between p-3 rounded-xl border border-border/50 hover:border-primary/20 transition-all hover:bg-primary/5 group">
                <div className="flex items-center gap-3">
