...
const AdminAppointmentNew = () => {
    ...
    return (<div className="min-h-screen bg-[#F7F9FC] -m-6 p-6 md:p-10 flex justify-center">
      <div className="w-full max-w-[850px]">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <Button variant="ghost" size="sm" onClick={() => navigate("/admin/appointments")} className="self-start md:self-center text-slate-500 hover:text-slate-900 hover:bg-slate-100/50 rounded-full pr-4">
          ...
          <form onSubmit={submit} className="divide-y divide-slate-100">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Select value={form.patient_id} onValueChange={(v) => setForm({ ...form, patient_id: v })}>
                    ...
                    <SelectContent>
                      {patients.map((p) => (<SelectItem key={p.id} value={p.id}>{p.full_name || p.id.slice(0, 8)}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
            ...
            <div className="p-6 md:p-8 space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Input type="date" value={form.appointment_date} className="h-11 rounded-xl bg-slate-50 border-slate-200 focus:ring-primary/20" onChange={(e) => setForm({ ...form, appointment_date: e.target.value })}/>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input type="time" value={form.appointment_time} className="h-11 rounded-xl bg-slate-50 border-slate-200 focus:ring-primary/20" onChange={(e) => setForm({ ...form, appointment_time: e.target.value })}/>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 md:p-8 space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Popover open={serviceSearchOpen} onOpenChange={setServiceSearchOpen}>
                    <PopoverTrigger asChild>
                      <Button variant="outline" role="combobox" className="w-full h-11 justify-between rounded-xl bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100/50">
                      ...
                      <Command>
                        ...
                        <CommandList>
                          ...
                          <CommandGroup heading="Clinic Services">
                            {services.map((s) => (<CommandItem key={s.id} value={s.name} onSelect={() => handleServiceSelect(s)} className="py-3 px-4 flex items-center justify-between cursor-pointer">
                                <div>
                          ...
                          <CommandGroup heading="Custom Option">
                            <CommandItem value="Other / Custom Consultation" onSelect={() => handleServiceSelect("custom")} className="py-3 px-4 flex items-center justify-between cursor-pointer">
                              <div>
                ...
                {(!form.service_id || form.service_id === "") && (<div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                    <Label className="text-slate-700 font-medium">Consultation Reason <span className="text-rose-500">*</span></Label>
                    <Input value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} placeholder="e.g., General dental checkup" className="h-11 rounded-xl bg-slate-50 border-slate-200 focus:ring-primary/20"/>
                  </div>)}
              </div>

              {selectedService && (<div className="bg-primary/5 rounded-xl p-4 border border-primary/10 flex gap-4 animate-in fade-in zoom-in-95">
              ...
              {(!form.service_id || form.service_id === "") && form.reason && (<div className="space-y-2">
                  <Label className="text-slate-700 font-medium">Additional Details</Label>
                  <Textarea value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} placeholder="Describe the consultation, cleaning, follow-up, or any specific patient concerns..." className="min-h-[100px] rounded-xl bg-slate-50 border-slate-200 focus:ring-primary/20 resize-none p-4 text-[15px]"/>
                </div>)}
            </div>

            {/* Action Buttons */}
            <div className="p-6 md:p-8 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-end gap-3">
              <Button type="button" variant="outline" onClick={() => navigate("/admin/appointments")} className="w-full sm:w-auto h-11 px-8 border-slate-200 text-slate-600 hover:bg-white rounded-xl transition-all">
                Cancel
              </Button>
              <Button type="submit" disabled={saving} className="w-full sm:w-auto h-11 px-8 bg-primary hover:bg-primary/90 text-white rounded-xl shadow-md shadow-primary/10 transition-all flex items-center justify-center gap-2">
                {saving ? ("Scheduling...") : (<>
