...
export const ExpertiseSection = ({ blocks }: ExpertiseSectionProps) => {
  // If we have CMS blocks, we use the first one as the primary expertise section content
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            <div className="relative">
              <div className="aspect-square rounded-[40px] overflow-hidden shadow-2xl">
                <img 
                  src={resolveImage(cmsBlock?.image_url) || "https://images.unsplash.com/photo-1579684385127-1ef15d508118?q=80&w=2000&auto=format&fit=crop"} 
                  alt={cmsBlock?.title || "Advanced Dental Technology"} 
              ...
              <div className="absolute -right-8 top-12 bg-white p-8 rounded-3xl shadow-xl border border-slate-100 hidden sm:block">
                <div className="flex flex-col gap-4">
                  {(cmsBlock?.items || ["3D Imaging", "Digital CAD/CAM", "Laser Precision"]).map((item, i) => (
          ...
          >
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">
              {cmsBlock?.subtitle || "Technology"}
            </span>
            <h2 className="mt-4 text-[40px] md:text-[52px] font-bold tracking-tight text-slate-900 leading-[1.1]">
              {cmsBlock?.title || <>Advanced technology.<br />Exceptional precision.</>}
            </h2>
            <p className="mt-8 text-lg text-slate-500 leading-relaxed">
              {cmsBlock?.body || "We invest in the latest dental innovations to provide you with more accurate diagnoses, faster treatments, and superior comfort. Our clinic is equipped with state-of-the-art systems designed for patient-centered excellence."}
            </p>
            
            {/* Standard static features if no specialized items provided */}
            {!cmsBlock && (
